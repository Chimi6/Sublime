...
- 2026-09-26: `png -> bmp` on the photo shape misses the pass line against the `png` crate: 358 against 433 MB/s of decoded pixels in the recorded block, 2 to 17% behind across the session's runs (`DOCS/benchmarks/png-bmp.md`); the other seven lines pass, the flat decode and both encodes with margin. No tag until it passes or the pair is left out. In-process the decode is 95 ms against the crate's 78 on 61 MB of pixels: the inflater's literal path (69 ms, a table lookup per one to three literals with an 8-cycle dependent chain) against fdeflate, and the checksums and unfilter, which the crate does with SIMD intrinsics the main crate cannot use (`unsafe` is forbidden; only auto-vectorization is open, and the Adler-32 loop does not vectorize on the baseline x86-64 target, which has no 32-bit vector multiply). Levers tried: a 13-bit table (3%), Adler-32 as 16-bit lanes (no change), and the x86-64-v2 baseline (`-C target-cpu=x86-64-v2`, measured 2026-09-26: the compiler emitted SSE4.1 across the binary and the line stayed at 377 against 440; reverted). What is left is `unsafe`: explicit SIMD for the checksums and unfilters and unchecked writes in the inflater, which the crate forbids; a scoped exception for the two checksums is the candidate spike, the inflater stays safe.
