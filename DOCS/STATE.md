...
- 2026-09-24: Apple Pages output (writer, branch `pages-writer`): `src/io/pages/writer.rs` writes a `.pages` by rewriting a template's body storage, mapping paragraphs to Pages' named styles (Title, Heading, Body) and pointing bold/italic runs at the template's character styles. The two rules Pages enforces silently (learned by comparing against a hand-authored fixture, verified by screenshotting the output in Pages): the bold/italic toggle alone does not render — its styles carry the weighted font — and the character-style table must start at offset 0 with the template's declared base style, or Pages drops the whole table. `markdown`, `text`, `html`, `docx` -> `pages`, lossy. Output opens in Pages 12. Bulleted and numbered lists reuse the theme's live list styles, and tables reuse the template's own tables (Pages requires each table registered in the calculation engine, which cannot be synthesized, so the writer rewrites a template table's cells and grid). Links become clickable hyperlinks (a `TSWP.HyperlinkFieldArchive` object per link range added to `Index/Document.iwa`, anchored by a `table_smartfield` table; verified by an exported PDF carrying every URI). Inline images reuse the template's own image (bytes swapped into its data files, sizes rewritten from the picture's PNG/JPEG header, the SHA-1 digest recomputed so Pages does not serve a cached original, the attachment made inline); `docx -> pages` carries them. Character styles now come from the ones the template body uses (its live emphasis variations), not the stylesheet definitions, which open but do not render. The template (`style_template.pages`) is a real Pages doc carrying named styles, list styles, a table, and an image, all reused. Next quality steps: a benchmark pair (goal-based, no tool writes Pages) and the release.
