...
- 2026-09-26: `png -> jpeg` on the photo shape is a tie the harness scores as a miss: 440 against jpeg-encoder's 445 MB/s in the recorded block (`DOCS/benchmarks/jpeg-png.md`), 449 against 448 the block before, 163 against 164 ms in seven direct runs each. The other seven lines pass. Spikes taken: reciprocal quantization (a third of the encode, gone), fused and wider bit writes, row-slice gathers; eight-lane transforms and lane-form color conversion measured slower and were reverted. The owner decides whether a tie ships.
