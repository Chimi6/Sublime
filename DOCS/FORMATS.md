...
| data | json | JSON | json | csv-to-json, toml-to-json, yaml-to-json, xml-to-json | json-to-csv, json-to-toml, json-to-yaml, json-to-xml |
| data | toml | TOML | toml | json-to-toml | toml-to-json |
| data | xml | XML | xml | json-to-xml | xml-to-json |
...
| toml | xml | conditional | toml-to-json -> json-to-xml | dates, times, infinities, and NaN become strings; @-keys become attributes, #text the text, arrays repeated elements; the root object must have one member, otherwise it is wrapped in <root>; numbers, booleans, and nulls become text |
...
| xml | toml | conditional | xml-to-json -> json-to-toml | attributes become @-keys, text #text, repeated elements arrays, every value a string; comments, processing instructions, the doctype, and the order of text against elements in mixed content are dropped; the root must be an object; nulls are dropped; integers beyond 64 bits become floats |
| xml | yaml | conditional | xml-to-json -> json-to-yaml | attributes become @-keys, text #text, repeated elements arrays, every value a string; comments, processing instructions, the doctype, and the order of text against elements in mixed content are dropped |
...
| yaml | xml | conditional | yaml-to-json -> json-to-xml | anchors are expanded, tags outside the core schema are dropped, keys become strings, infinities and NaN become strings, a multi-document stream becomes an array; @-keys become attributes, #text the text, arrays repeated elements; the root object must have one member, otherwise it is wrapped in <root>; numbers, booleans, and nulls become text |
