...
fn generate_xml(units: &str, path: &str) -> Result<(), String> {
    ...
    writeln!(writer, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<records version=\"3\">").map_err(|error| error.to_string())?;
    for index in 0..count {
        let note = if index % 7 == 0 { "with &amp; and &lt;angles&gt;" } else { "plain" };
        writeln!(
            writer,
...
fn generate_xml_prose(units: &str, path: &str) -> Result<(), String> {
    let count = parse_rows(units)?;
    let mut writer = open_output(path)?;
    writeln!(writer, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<sections>").map_err(|error| error.to_string())?;
...
fn generate_json(units: &str, path: &str) -> Result<(), String> {
    let count = parse_rows(units)?;
    let mut writer = open_output(path)?;
    write!(writer, "{{\"records\":{{\"@version\":\"3\",\"record\":[").map_err(|error| error.to_string())?;
    for index in 0..count {
        if index > 0 {
...
fn finish_frame(frame: Frame) -> (String, serde_json::Value) {
    ...
    if !text.is_empty() {
        members.insert("#text".to_string(), serde_json::Value::String(text.to_string()));
    }
    (frame.name, serde_json::Value::Object(members))
}

fn add_child(members: &mut serde_json::Map<String, serde_json::Value>, name: String, child: serde_json::Value) {
...
fn crates_xml_to_json(input: &str, output: &str) -> Result<(), String> {
    ...
    loop {
        let event = reader.read_event_into(&mut buffer).map_err(|error| error.to_string())?;
        ...
        match event {
            Event::Start(start) | Event::Empty(start) => {
                ...
                for attribute in start.attributes() {
                    ...
                    let value = attribute.unescape_value().map_err(|error| error.to_string())?.to_string();
    ...
    serde_json::to_writer(&mut writer, &serde_json::Value::Object(document)).map_err(|error| error.to_string())?;
    writer.flush().map_err(|error| error.to_string())
}
    ...
    match value {
        ...
        serde_json::Value::Object(members) => {
            ...
            if let Some(text) = text {
                writer.write_event(Event::Text(BytesText::new(&text))).map_err(|error| error.to_string())?;
            }
            for (key, child) in children {
                write_element(writer, key, child)?;
            }
            writer.write_event(Event::End(BytesEnd::new(name))).map_err(|error| error.to_string())
        }
        scalar => {
            let text = scalar_text(scalar);
            if text.is_empty() {
                writer.write_event(Event::Empty(BytesStart::new(name))).map_err(|error| error.to_string())
            } else {
                writer.write_event(Event::Start(BytesStart::new(name))).map_err(|error| error.to_string())?;
                writer.write_event(Event::Text(BytesText::new(&text))).map_err(|error| error.to_string())?;
                writer.write_event(Event::End(BytesEnd::new(name))).map_err(|error| error.to_string())
            }
        }
...
fn crates_json_to_xml(input: &str, output: &str) -> Result<(), String> {
    let file = File::open(input).map_err(|error| error.to_string())?;
    let value: serde_json::Value = serde_json::from_reader(BufReader::new(file)).map_err(|error| error.to_string())?;
