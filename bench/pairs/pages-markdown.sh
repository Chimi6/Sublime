#!/usr/bin/env bash
# Pages -> markdown. Sourced by bench/run.sh. Inputs and goals come from the
...
run_pair() {
  ...
  for name in styled prose; do
    ...
    ours="$(time_cmd ours "$sublime" -q convert "$input" "$data/$name.md" --to markdown)"
    pages_rows pages markdown "$name" "$bytes" "$ours"
  done
}
