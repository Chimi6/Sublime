...
run_pair() {
  ...
  for name in styled prose; do
    ...
    row "pages -> html throughput, $name (MB/s of the package)" "$(mbps "$bytes" "$os")" "$(mbps "$bytes" "$rs") (pages-json)" "$(pass "$(echo "$os <= $rs" | bc -l)")"
    row "Peak RSS pages -> html, $name (MB)" "$(rss_mb "$(rss_of "$ours")")" "$(rss_mb "$(rss_of "$reference")") (pages-json)" "$(pass "$(echo "$(rss_of "$ours") <= $(rss_of "$reference")" | bc -l)")"
  done
}
