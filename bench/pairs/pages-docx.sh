#!/usr/bin/env bash
# Pages -> docx. Sourced by bench/run.sh. Inputs are the pages-json pair's
...
run_pair() {
  ...
  for name in styled prose; do
    ...
    row "pages -> docx, $name ($(mb "$bytes") MB): throughput (MB/s of input)" "$(mbps "$bytes" "$os")" "$(mbps "$bytes" "$rs") our pages -> pages-json on the same input; line: not slower" "$(pass "$(echo "$os <= $rs" | bc -l)")"
    row "pages -> docx, $name: peak memory (MB)" "$(rss_mb "$(rss_of "$ours")")" "$(rss_mb "$(rss_of "$reference")") our pages -> pages-json on the same input; line: not more" "$(pass "$(echo "$(rss_of "$ours") <= $(rss_of "$reference")" | bc -l)")"
  done
}
